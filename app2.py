...
#             if eco_mode:
#                 result = call_novalite_api(sys_msg, user_prompt)
#             else:
...
#             if eco_mode:
#                 refine_output = call_novalite_api(refine_sys, refine_user)
#             else:
...
#             if eco_mode:
#                 new_summary = call_novalite_api(merge_sys, merge_user)
#             else:
...
#         if eco_mode:
#             doc_text = call_novalite_api(sys_msg, user_prompt)
#         else:
...
#             return call_claude_api(sys_msg, user_prompt)
#         elif model_name == "Nova Lite":
#             return call_novalite_api(sys_msg, user_prompt)
#         elif model_name == "Deepseek R1":
#             return call_deepseek_api(sys_msg, user_prompt)
#         elif model_name == "GPT 4o":
#             return call_gpt_api(sys_msg, user_prompt)
#         else:
#             # Fallback
#             return call_llm_api(sys_msg, user_prompt)
...
#             eco_mode=eco_mode,
#             max_iterations=2  # You can customize this
#         )
...
def query_documents_with_page_range(
    ...
    """
    An upgraded multi-step Deep Research workflow with true iterative generation:
     1. Retrieve relevant doc chunks (FAISS).
     2. Perform iterative chunk-based analysis with repeated web searches (if analyse_mode=True).
     3. If draft_mode=True => produce a structured, chapter-wise "research paper" style doc.
        Else => produce a final Q&A style answer.
     4. All long-form content is generated iteratively in segments with context preservation.
    ...
    ##############################################################################
    # 3) Helper: LLM Call based on selected model
    ##############################################################################
    ...
    ##############################################################################
    # 5) Iterative Chapter-Wise Drafting Agent (for `draft_mode=True`)
    ##############################################################################
    def drafting_agent_chapterwise(
        user_query: str,
        final_analysis_text: str,
        ...
        """
        Produces a "research paper" style structured doc through iterative generation:
         1) Generate outline
         2) Generate each chapter separately
         3) Combine into final document
        
        Uses multiple LLM calls to handle token limitations.
        """
        # Step 1: Generate outline with chapter structure
        outline_sys = (
            "You are a drafting agent. Based on the analysis text and references provided, "
            "create a detailed outline for a multi-chapter research paper. Include chapter titles, "
            "section headings, and brief descriptions of what each section should cover."
        )
        ...
        {final_analysis_text[:2000]}... [truncated for outline generation]

        Please create a detailed outline for a professional legal research paper with:
        1. Table of Contents
        2. Introduction
        3. Multiple analysis chapters (determine appropriate number based on content)
        ...
        if not chapter_markers or len(chapter_markers) < 3:  # Fallback if parsing fails
            chapter_markers = ["Introduction", "Analysis", "Findings", "Conclusion"]
        
        # Add standard sections
        all_sections = ["Table of Contents", "Introduction"] + chapter_markers + ["Conclusion"]
        
        # Step 3: Generate each section iteratively
        generated_sections = {}
        toc = ""
        full_document = ""
        
        # First generate TOC
        toc_sys = "You are a drafting agent. Create a detailed table of contents for a research paper based on the outline."
        toc_user = f"OUTLINE:\n{outline}\n\nCreate a formal Table of Contents with page numbers."
        toc = call_selected_model(toc_sys, toc_user)
        generated_sections["Table of Contents"] = toc
        full_document += f"# Table of Contents\n\n{toc}\n\n"
        
        # Then generate each section with context from previous sections
        for i, section in enumerate(all_sections[1:], 1):  # Skip TOC as we already generated it
            section_sys = (
                f"You are a drafting agent writing the '{section}' section of a legal research paper. "
                f"Write a detailed, comprehensive section that maintains continuity with previously written sections."
            )
            
            # Include progressively more context as we generate more sections
            context_sections = ""
            ...
            for prev_i in range(max(0, i-3), i):
                ...
                if prev_section in generated_sections:
                    context_sections += f"# {prev_section}\n{generated_sections[prev_section]}\n\n"
            
            # Determine what portion of analysis text to include based on section
            if section == "Introduction":
                # For intro, just use beginning of analysis
                analysis_excerpt = final_analysis_text[:3000]
            elif section == "Conclusion":
                # For conclusion, use end of analysis
                analysis_excerpt = final_analysis_text[-5000:]
            else:
                # For middle sections, try to use relevant portions
                # This is simplified - ideally you'd have a more sophisticated approach
                section_index = all_sections.index(section)
                section_count = len(all_sections) - 2  # Excluding TOC and intro
                if section_count <= 1:
                    analysis_excerpt = final_analysis_text
                else:
                    chunk_size = len(final_analysis_text) // section_count
                    start_idx = (section_index - 2) * chunk_size  # -2 to account for TOC and intro
            ...
            REFERENCES:
            {json.dumps(references[:5], indent=2)}  # Limit references for token reasons
            
            WEB DATA SAMPLE:
            {json.dumps(all_web_data[:2], indent=2) if all_web_data else "No web data available"}
            
            Write a detailed, comprehensive '{section}' section that maintains continuity with previous sections.
            Focus on professional legal analysis with proper citations to sources.
            """
            
            section_content = call_selected_model(section_sys, section_user)
            generated_sections[section] = section_content
            
            # For longer sections, we may need to generate them in parts
            if len(section_content) > 8000 and section not in ["Introduction", "Conclusion", "Table of Contents"]:
                # Split into subsections
                subsection_sys = (
                    f"You are continuing to write the '{section}' section of a legal research paper. "
                    f"The first part has been written. Now write the next part, maintaining continuity."
                )
                
                subsection_user = f"""
                SECTION: {section}
                
                FIRST PART ALREADY WRITTEN:
                {section_content}
                
                RELEVANT ANALYSIS TEXT:
                {analysis_excerpt[4000:]}
                
                Continue writing this section, adding more detailed analysis, examples, and insights.
                Maintain the same professional tone and citation style.
                """
                
                additional_content = call_selected_model(subsection_sys, subsection_user)
                section_content += "\n\n" + additional_content
                generated_sections[section] = section_content
        ...
        format_sys = (
            "You are a document formatting specialist. Review this research paper and ensure "
            "consistent formatting, proper citations, and smooth transitions between sections."
        )
        
        # We can't pass the entire document for formatting as it might exceed token limits
        # Instead, do a final pass on just the structure and transitions
        format_user = f"""
        DOCUMENT STRUCTURE:
        {outline}
        
        SECTION TRANSITIONS TO CHECK:
        {toc}
        
        The first paragraph of each main section:
        {'; '.join([f"{s}: {generated_sections[s][:200]}..." for s in generated_sections])}
        
        Please provide any formatting corrections or transition improvements needed.
        """
        
        format_suggestions = call_selected_model(format_sys, format_user)
        
        # We don't actually apply these automatically, but in a more sophisticated system,
        # you could parse and apply the formatting suggestions
        
        return full_document

    ##############################################################################
    # 6) Iterative Q&A Agent (if draft_mode=False)
    ##############################################################################
    def qna_agent_iterative(
        user_query: str,
        final_analysis_text: str,
        references: list,
        last_msgs: list,
        ...
        """
        Return a comprehensive Q&A style answer through iterative generation.
        Builds the answer in segments to handle token limitations.
        """
        # Step 1: Generate initial answer based on first portion of analysis
        initial_sys = (
            "You are a legal analysis Q&A agent. Provide a detailed, comprehensive answer "
            "to the user's query based on the analysis text and references provided. "
            "This is the first part of your answer - focus on introducing the topic and "
            "beginning your analysis."
        )
        
        # Use first portion of analysis text
        text_limit = min(len(final_analysis_text), 6000)
        initial_user = f"""
        USER QUERY:
        {user_query}

        ANALYSIS TEXT (PART 1):
        {final_analysis_text[:text_limit]}
        ...
        Begin your comprehensive answer. This is part 1 of a multi-part response.
        Focus on introducing the topic and starting your detailed analysis.
        """
        
        answer_part1 = call_selected_model(initial_sys, initial_user)
        
        # Step 2: Continue with middle portion of analysis
        if len(final_analysis_text) > text_limit:
            middle_sys = (
                "You are continuing a detailed legal analysis. Based on the previous part of your answer "
                "and additional analysis text, continue developing your comprehensive response. "
                "Maintain continuity with what you've already written."
            )
            
            middle_text_start = text_limit
            middle_text_end = min(len(final_analysis_text), middle_text_start + 6000)
            
            middle_user = f"""
            USER QUERY:
            {user_query}

            YOUR PREVIOUS ANSWER (PART 1):
            {answer_part1}

            ANALYSIS TEXT (PART 2):
            {final_analysis_text[middle_text_start:middle_text_end]}

            ADDITIONAL REFERENCES:
            {json.dumps(references[5:10], indent=2) if len(references) > 5 else "No additional references"}

            Continue your comprehensive answer. This is part 2 of your response.
            Build upon your previous points and develop the analysis further.
            """
            
            answer_part2 = call_selected_model(middle_sys, middle_user)
            full_answer = answer_part1 + "\n\n" + answer_part2
            
            # Step 3: If needed, add conclusion with final portion of analysis
            if len(final_analysis_text) > middle_text_end:
                conclusion_sys = (
                    "You are concluding a detailed legal analysis. Based on your previous answer parts "
                    "and the final portion of analysis text, provide a strong conclusion to your comprehensive response. "
                    "Summarize key points and offer final insights."
                )
                ...
                YOUR PREVIOUS ANSWER (PARTS 1-2):
                {full_answer[-4000:]}  # Just use the end for context

                ANALYSIS TEXT (FINAL PART):
                {final_analysis_text[middle_text_end:]}

                REMAINING REFERENCES:
                {json.dumps(references[10:], indent=2) if len(references) > 10 else "No additional references"}

                Conclude your comprehensive answer. Summarize key points, provide final insights,
                and ensure your response fully addresses the original query.
                """
                
                answer_part3 = call_selected_model(conclusion_sys, conclusion_user)
    ...
    # C) Output
    if draft_mode:
        # Produce a "research paper" style doc using iterative generation
        doc_text = drafting_agent_chapterwise(
            user_query=prompt,
            final_analysis_text=final_analysis_text,
            references=references,
        ...
        # Comprehensive Q&A using iterative generation
        answer = qna_agent_iterative(
            user_query=prompt,
            final_analysis_text=final_analysis_text,
            references=references,
            last_msgs=last_messages,
